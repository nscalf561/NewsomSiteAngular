...
///////////////////////////////////////////////////////////////////
//                                                               //
//  ULTIMATE DROP DOWN MENU Version 4.5 Lite by Brothercake      //
//  http://www.udm4.com/                                         //
//                                                               //
...
if(umTree&&um.d&&!um.mie){if(g){um.ap('000',umTree);
}for(i in um.menuCode){var l=um.gd(i);
if(l){if(typeof(l)!='object'){l.innerHTML+=um.menuCode[i];}
}}um.bub=0;
um.wsr=0;
...
}else{l.onmouseover=function(){um.n.lr(um.gc(l),1);
um.bub=0;
};
}l[h]=function(e){var v=(um.ie)?window.event.srcElement:e.target;
...
l.out=function(e){if(um.o7&&um.ha&&um.kdf){return;
}if(um.lf){um.gc(this).blur();
}um.lf=0;
if(!e){e=window.event;
e.relatedTarget=e.toElement;
}if(!l.contains(e.relatedTarget)){if(!um.tr.contains(e.relatedTarget)){clearInterval(um.cc);
um.cr=0;
}um.n.cp(umM,l);
...
umNav.prototype.cd=function(m){var s=um.gu(m);
var n=s.length;
for(var i=0;
i<n;
i++){this.clm(s[i]);
}};
umNav.prototype.ck=function(m){var l=m.getElementsByTagName('a');
var n=l.length;
for(var i=0;
i<n;
i++){this.lr(l[i],0);
...
var x=0;
var y=0;
if(!(um.h&&s)){x=(s)?(um.a?(0-p.mw):p.pw):((um.a?(0-p.mw):p.pw)-um.e[51]-um.e[55]);
y=(0-p.th);
